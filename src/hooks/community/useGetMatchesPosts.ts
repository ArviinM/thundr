...
export function useGetMatchesPosts(props: GetFeedRequest) {
  const axiosInstance = useAxiosWithAuth();

  return useInfiniteQuery({
    queryKey: ['get-matches-post', props],
    ...
    queryFn: async ({pageParam = null}): Promise<FeedResponse[]> => {
      ...
      const response: AxiosResponse<BaseResponse<FeedResponse[]>> =
        await axiosInstance.get('/community/match-posts', config);

      if (response.status !== HttpStatusCode.Ok) {
        showErrorToast({
          name: 'get-matches-post',
